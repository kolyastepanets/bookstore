  ...
  describe 'POST #create' do
    ...
    context 'with valid atrributes' do
      it 'saves new review with book_id' do
        expect { post :create, book_id: book, review: attributes_for(:review) }.to change(Review, :count).by(1)
      end
    ...
    context 'with invalid attributes' do
      it 'does not save review' do
        expect { post :create, book_id: book, review: attributes_for(:review, :invalid) }.to_not change(Review, :count)
      end

      it 'redirect to #new' do
        post :create, book_id: book, review: attributes_for(:review, :invalid)
        expect(response).to render_template :new
      end
    end
  end
